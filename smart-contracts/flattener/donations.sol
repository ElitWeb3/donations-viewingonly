...
contract DonationsICO is Ownable,ReentrancyGuard {
  using SafeMath for uint256;
    ...
    function donateAndBuyNFT(address beneficiary,uint256 _mintAmount,string memory _donationType) public payable nonReentrant {
        ...
        require(weiAmount == donationsData[_donationType].donationAmount,"amount must be equal to donation amount");
        donationsData[_donationType].accountAddress.send(weiAmount);
    ...
    function defineDonations(uint256[] memory _donationsAmount,string[] memory _donationsFor,address[] memory _address,uint256[] memory _donationsId, bool isPercentage) public onlyOwner{
        require(
